...
SFT_CKPT="${BASE_CKPT:-/data/checkpoints/n1d7_sft_from_base_20k/n1d7_sft_from_base_20k/checkpoint-20000}"
BACKBONE="/data/checkpoints/Cosmos-Reason2-2B"
MODALITY="/data/datasets/libero_10_no_noops_lerobot/meta/modality.json"
STATUS=/tmp/recap20k_iter_status.log
...
for R in $(seq 1 "$NUM_ROUNDS"); do
  ...
  PKL="/data/datasets/recap20k_iter${R}_pkl"
  REPOID="recap20k_iter${R}"
  DS="/data/datasets/${REPOID}"
  VALDIR="/data/checkpoints/recap20k_iter${R}_value"
  CFGEXP="recap20k_iter${R}_${CONDITIONING}"
  ...
  EVLOG="/tmp/n17_cfg_eval_20k_iter${R}.log"
  ...
  ( source "$OPENPI_ACT"; common_env; cd "$REPO/examples/recap/process"
    python compute_returns.py --config-name compute_returns_rollout \
      "data.train_data_paths.0.dataset_path=$DS" data.tag=rollout 2>&1 | tee /tmp/recap20k_iter${R}_returns.log | tail -2 )
  RMIN=$(grep -oE "return: min=-?[0-9.]+" /tmp/recap20k_iter${R}_returns.log | tail -1 | grep -oE -- "-?[0-9.]+")
  RMIN=${RMIN:--811.0}
  ...
  if [ -f "$DS/meta/advantages_rollout.parquet" ]; then
    ...
    ( source "$OPENPI_ACT"; common_env; cd "$REPO/examples/recap/process"
      # workers=0: value model is on GPU before the DataLoader forks, so
      # num_workers>0 forks CUDA-initialized workers that deadlock on the first
      # batch (observed: 7.5h hang). Single-process loading avoids the fork.
  ...
  if [ -f "$NEWCK" ]; then
    ...
    ( source "$N1D7_ACT"; common_env; cd "$REPO/examples/recap/cfg"
      python train_cfg.py --config-name "$CFG_CONFIG" \
        ...
        "runner.max_steps=$CFG_STEPS" "runner.save_interval=500" \
  ...
  if grep -q "SUMMARY total_sr" "$EVLOG" 2>/dev/null; then
    ...
    ( source "$N1D7_ACT"; common_env
      bash "$REPO/evaluations/libero/eval_n1d7_cfg_ckpt.sh" "$NEWCK" "20k_iter${R}" "$GUIDANCE" "$CONDITIONING" 2>&1 | tail -2 )
