...
def _register_builtin_models():
    ...
    def _build_gr00t_cfg(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.gr00t import get_model

        return get_model(cfg, torch_dtype)

    def _build_gr00t_n1d7_cfg(cfg: DictConfig, torch_dtype):
        from rlinf.models.embodiment.gr00t import get_model
    ...
    register_model(
        SupportedModel.GR00T_CFG.value,
        _build_gr00t_cfg,
        category="embodied",
        force=True,
    )
    register_model(
        SupportedModel.GR00T_N1D7_CFG.value,
        _build_gr00t_n1d7_cfg,
        category="embodied",
        force=True,
    )
